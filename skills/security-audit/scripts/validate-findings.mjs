...
// identical to docs/security-audit/studio/lib/validate-findings.mjs in the
// auditoria project (Task 1) — keep the two in sync by hand if these rules
// change. Masks real secrets/emails before findings reach the public
// dashboard (see redactSecrets below); placeholder/example values used to
// illustrate a finding are left untouched.
...
function walkStrings(node, path, cb) {
  ...
  else if (Array.isArray(node)) node.forEach((item, i) => walkStrings(item, `${path}[${i}]`, cb))
  else if (node !== null && typeof node === 'object') {
    for (const [key, value] of Object.entries(node)) walkStrings(value, path ? `${path}.${key}` : key, cb)
  }
}
...
export function scanForSecrets(data) {
  ...
  walkStrings(data, '', (str, path) => {
    ...
    for (const match of str.matchAll(SECRET_ASSIGNMENT_RE)) {
      if (!isPlaceholderValue(match[1])) hits.push({ path, rule: 'secret-assignment' })
    }
  })
  return hits
}
...
export function validateFindings(data) {
  ...
  if (typeof data.audit_date !== 'string' || data.audit_date.length === 0 || !/^\d{4}-\d{2}-\d{2}$/.test(data.audit_date)) errors.push(`audit_date: "${data.audit_date}" must match YYYY-MM-DD`)
  if (Array.isArray(data.categories)) {
    const seen = new Set()
    for (const cat of data.categories) {
      if (!CATEGORY_IDS.includes(cat.id)) { errors.push(`categories: unknown id ${cat.id}`); continue }
      seen.add(cat.id)
      if (cat.name !== CATEGORY_NAMES[cat.id]) errors.push(`categories[${cat.id}]: name must be "${CATEGORY_NAMES[cat.id]}"`)
      ...
      if (cat.applicable === false && (typeof cat.na_reason !== 'string' || cat.na_reason.trim().length === 0)) errors.push(`categories[${cat.id}]: na_reason required (non-empty string) when applicable is false`)
    }
  ...
  for (const arrName of ['scans', 'sbom']) {
    if (Array.isArray(data[arrName])) {
      for (const [i, item] of data[arrName].entries()) {
        if (typeof item !== 'object' || item === null || Array.isArray(item) || Object.keys(item).length === 0) errors.push(`${arrName}[${i}]: expected a non-empty object`)
      }
    } else errors.push(`${arrName}: expected an array`)
  }
